...
const panel = "rounded-lg border border-gray-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800";
...
const resolvePlayerFoto = (playerFoto) => {
  ...
  if (trimmed.startsWith("/players/uploads-players/")) return `${API_BASE_URL}${trimmed}`;
  if (trimmed.startsWith("/uploads-players/")) return `${API_BASE_URL}${trimmed}`;
...
export default function PublicPlayers() {
  ...
  const hasActiveFilters = filters.search.trim() !== "" || filters.sporti_id !== "";

  const filteredPlayers = useMemo(() => {
    return players.filter((player) => {
      const searchMatch = filters.search === "" || 
        `${player.emri} ${player.mbiemri}`.toLowerCase().includes(filters.search.toLowerCase()) ||
        player.ekipi_emri.toLowerCase().includes(filters.search.toLowerCase());
      
      const sportMatch = filters.sporti_id === "" || String(player.sporti_id) === String(filters.sporti_id);
        ...
        >
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm text-blue-100">
                  Browse through all the players grouped by their respective sports and teams.
                </p>
              </div>
            </div>
          </div>
          ...
          >
            <motion.div variants={statCardVariants} className="rounded-lg border border-gray-200 p-4 dark:border-slate-700">
              <p className={`text-xs font-bold uppercase ${mutedText}`}>Total Players</p>
              <p className={`mt-1 text-2xl font-black ${strongText}`}>{players.length}</p>
            </motion.div>
            <motion.div variants={statCardVariants} className="rounded-lg border border-gray-200 p-4 dark:border-slate-700">
              <p className={`text-xs font-bold uppercase ${mutedText}`}>Active Teams</p>
              <p className={`mt-1 text-2xl font-black ${strongText}`}>
                {new Set(players.filter(p => p.team_id).map(p => p.team_id)).size}
              </p>
            </motion.div>
            <motion.div variants={statCardVariants} className="rounded-lg border border-gray-200 p-4 dark:border-slate-700">
              <p className={`text-xs font-bold uppercase ${mutedText}`}>Sports</p>
              <p className={`mt-1 text-2xl font-black ${strongText}`}>
                {new Set(players.filter(p => p.sporti_id).map(p => p.sporti_id)).size}
              </p>
            </motion.div>
          </motion.div>
        </motion.section>
        ...
        {!error && Object.keys(groupedPlayers).length === 0 ? (
           <div className={`${panel} p-8 text-center`}>
             ...
             <p className={`mx-auto mt-1 max-w-xl text-sm ${mutedText}`}>
               {hasActiveFilters ? "Try adjusting your filters or search query." : "There are currently no players registered."}
             </p>
           </div>
        ) : (
              ...
              {Object.entries(groupedPlayers).map(([sportName, sportData]) => (
                ...
                >
                  ...
                  <div className="grid gap-6">
                    {Object.entries(sportData.teams).map(([teamName, teamPlayers]) => (
                      <div key={teamName} className={`${panel} overflow-hidden`}>
                        ...
                        <div className="grid gap-px bg-gray-200 dark:bg-slate-700 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                          {teamPlayers.map((player) => (
                            <div key={player.id} className="bg-white dark:bg-slate-800 p-4 flex flex-col items-center text-center transition-colors hover:bg-gray-50 dark:hover:bg-slate-800/60">
                                ...
                                {player.foto ? (
                                  <img 
                                    ...
                                    onError={(e) => {
                                      ...
                                      e.target.src = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 24 24' stroke='%239CA3AF'%3E%3Cpath stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z'/%3E%3C/svg%3E";
                                    }}
                                  />
                                ) : (
