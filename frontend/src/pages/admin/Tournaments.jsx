...
const statusOptions = ["Regjistrimi", "Aktiv", "Përfunduar", "Anuluar"];
...
const tournamentUpdateSchema = yup.object().shape({
  ...
  lloji: yup.string().oneOf(tournamentTypeOptions, "Invalid tournament format"),
...
/**
 * Normalizes the tournament form data by trimming text inputs.
 *
 * @param {Object} formData - The raw form data from state
 * @returns {Object} Normalized tournament form data
 */
...
/**
 * Returns Tailwind CSS class names for a tournament status badge.
 *
...
function getStatusBadgeClasses(status) {
  ...
  if (status === "Përfunduar")
    return "bg-gray-200 text-gray-700 dark:bg-slate-700 dark:text-slate-100";
  if (status === "Anuluar")
...
/**
 * Returns Tailwind CSS class names for a tournament format badge.
 *
 * @param {string} format - The format/type of the tournament
 * @returns {string} The CSS class string
 */
function getFormatBadgeClasses(format) {
  ...
  if (format === "Vetëm Eliminim")
    return "bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-100";
  return "bg-gray-100 text-gray-700 dark:bg-slate-700 dark:text-slate-100";
}

/**
 * Converts a raw date value into a YYYY-MM-DD string suitable for date input elements.
 *
 * @param {string|Date} value - The raw date value
 * @returns {string} The 10-character date string or an empty string
 */
...
/**
 * Formats a date string/value into en-GB format (DD/MM/YYYY).
 *
...
/**
 * Normalizes a raw user database record into a standardized user object.
 *
...
/**
 * Looks up the full name or username of a tournament organizer by ID.
 *
 * @param {Array} users - The list of user records
 * @param {number} organizerId - The ID of the organizer to find
 * @returns {string} The organizer's name or "N/A"
 */
...
/**
 * Checks if a user has the organizer role.
 *
...
/**
 * Checks if a user can be assigned as an organizer (must be either organizer or regular user).
 *
 * @param {Object} user - The user object
...
export default function Tournaments() {
    ...
    async (pageNum, filtersObj) => {
      ...
      try {
        ...
        const params = new URLSearchParams({
          ...
          ...(filtersObj.organizatori_id && {
            organizatori_id: filtersObj.organizatori_id,
          }),
        });
        ...
        const [tournamentsRes, sportsRes, usersRes] =
        ...
        setUsers(
          Array.isArray(usersPayload)
            ? usersPayload.map(normalizeUserRecord)
            : [],
        );
      } catch (err) {
  ...
  const handleEdit = (id) => {
    ...
    setFormData({
      ...
      organizatori_id: tournament.organizatori_id
        ? String(tournament.organizatori_id)
        : "",
  ...
  const buildPayload = () => ({
    ...
    organizatori_id:
      formData.organizatori_id === "" ? null : Number(formData.organizatori_id),
    cmimi_regjistrimit:
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.inner) {
        ...
        setAlert({
          type: "error",
          message: "Error creating tournament: " + message,
        });
      }
    }
  };
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      if (err.inner) {
        ...
        setAlert({
          type: "error",
          message: "Error updating tournament: " + message,
        });
      }
    }
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setAlert({
        type: "error",
        message: "Error deleting tournament: " + message,
      });
    }
  };
  ...
  return (
    <div className="bg-gray-50 dark:bg-slate-900 p-4">
      ...
      <div className="w-full mx-auto space-y-6">
        ...
        <div
          className={`flex bg-white dark:bg-slate-900 rounded-lg shadow-md overflow-x-auto ${loading ? `opacity-60 pointer-events-none` : ""}`}
        >
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-slate-700">
                {filteredTournaments.map((tournament) => (
                  <tr
                    key={tournament.id}
                    className="hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors duration-150"
                  >
                    <td className="px-4 py-3">
                      ...
                      <div className="text-sm text-gray-500 dark:text-slate-400">
                        Registration:{" "}
                        {formatCurrency(tournament.cmimi_regjistrimit)} EUR
                      </div>
                    </td>
              ...
              {isAdmin && (
                <div>
                  ...
                  <p className="text-gray-800 dark:text-slate-100 bg-gray-100 dark:bg-slate-700 px-4 py-2 rounded-lg">
                    {getOrganizerName(
                      users,
                      selectedTournament.organizatori_id,
                    )}
                  </p>
                </div>
              )}
