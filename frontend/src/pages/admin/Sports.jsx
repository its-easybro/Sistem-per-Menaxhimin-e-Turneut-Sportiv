...
// Initial form data for creating/editing sports.
...
// Defines the available sport types and period label options for form validation and dropdowns.
const sportTypeOptions = ["Ekipor", "Individual", "I dyfishtë"];

// Standard period labels that can be selected when creating or editing a sport, ensuring consistency across the application.
const periodLabelOptions = ["Half", "Quarter", "Period", "Set"];

// Validation schemas for creating and updating sports.
const sportCreateSchema = yup.object().shape({
  emertimi: yup
    .string()
    .min(2, "Sport name must be at least 2 characters")
    .required("Sport name is required"),
  pershkrimi: yup.string().nullable().notRequired(),
  numri_lojtareve: yup
    .number()
...
const sportUpdateSchema = yup.object().shape({
  emertimi: yup.string().min(2, "Sport name must be at least 2 characters"),
  pershkrimi: yup.string().nullable().notRequired(),
  numri_lojtareve: yup
    .number()
    .positive("Number of players must be positive")
    .nullable(),
  lloji: yup.string().oneOf(sportTypeOptions, "Invalid sport type"),
  kohezgjatja_default: yup
    .number()
    .positive("Default duration must be positive")
    .nullable(),
...
export default function SportsManagment() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.inner) {
        ...
        setAlert({
          type: "error",
          message: "Error creating sport: " + err.message,
        });
      }
    }
  };
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      if (err.inner) {
        ...
        setAlert({
          type: "error",
          message: "Error updating sport: " + err.message,
        });
      }
    }
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setAlert({
        type: "error",
        message: "Error deleting sport: " + err.message,
      });
    }
  };
                ...
                {filteredSports.length > 0 ? (
                  filteredSports.map((s) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-gray-700 dark:text-slate-300 font-semibold">
                        {s.numri_lojtareve}
                      </td>
                      <td className="px-6 py-4 text-gray-800 dark:text-slate-200 font-semibold">
                        <span
                          className={`px-3 py-1 rounded-full text-sm font-semibold ${getSportTypeBadgeClasses(s.lloji)}`}
                        >
                          {s.lloji || "-"}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-gray-700 dark:text-slate-300">
                        ...
                        <span className="block text-xs text-gray-500 dark:text-slate-400">
                          {s.numri_periodave ?? 2} x{" "}
                          {s.emri_periodave || "Half"}
                        </span>
                      </td>
                  ...
                  <tr>
                    ...
                    >
                      {hasActiveFilters
                        ? "No sports match these filters."
                        : 'No sports found. Click "Create New Sport" to add one.'}
                    </td>
                  </tr>
                )}
              </tbody>
                  ...
                  <div className="grid gap-4 md:grid-cols-3">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="90"
                      />
                      {formErrors.kohezgjatja_default && (
                        <p className="text-red-500 text-xs mt-1">
                          {formErrors.kohezgjatja_default}
                        </p>
                      )}
            ...
            >
              ...
              >
                ...
                <p className="text-gray-700 dark:text-slate-300 mb-6">
                  ...
                  <strong>{selectedSport.emertimi}</strong>? This action cannot
                  be undone.
                </p>
