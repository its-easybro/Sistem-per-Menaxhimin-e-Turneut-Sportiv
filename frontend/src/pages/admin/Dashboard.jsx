...
// Renders a comprehensive overview of the system for administrators, displaying stats, recent users, and quick actions.
const Dashboard = () => {
  ...
  useEffect(() => {
    /**
     * Fetches core statistics (total users, total sports, active sessions) from the API.
     * Triggers a parallel fetch to minimize load time and updates the dashboard state.
     */
    ...
    <div className="max-w-6xl mx-auto w-full">
      <header className="mb-8">
        ...
        <p className="text-gray-500 dark:text-slate-400 mt-1">
          Welcome back, {user?.emri || "Admin"}. Here is your system summary.
        </p>
      </header>
      {/* Stats Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm flex items-center gap-5 hover:shadow-md transition-all duration-300">
          ...
          <div>
            <p className="text-sm font-medium text-gray-500 dark:text-slate-400 transition-colors duration-300">
              Total Users
            </p>
            <h3 className="text-3xl font-black text-gray-900 dark:text-slate-200 transition-colors duration-300">
              {stats.users}
            </h3>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <table className="w-full text-left border-collapse min-w-[500px]">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-slate-700/50 text-sm">
                {recentUsers.map((u) => (
                  <tr
                    key={u.id || u.email}
                    className="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors duration-300"
                  >
                    <td className="px-4 py-4 font-medium text-gray-900 dark:text-slate-200 transition-colors duration-300">
                      {u.username || u.emri}
                    </td>
                    <td className="px-4 py-4 text-gray-500 dark:text-slate-400 transition-colors duration-300">
                      {u.email}
                    </td>
                    <td className="px-4 py-4">
                      <span
                        className={`px-2.5 py-1 rounded-full text-xs font-bold transition-colors duration-300 ${u.roli === "admin" ? "bg-purple-100 text-purple-700 dark:bg-purple-500/20 dark:text-purple-400" : u.roli === "organizator" ? "bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400" : u.roli === "gjyqtar" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400" : "bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-300"}`}
                      >
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm p-6 relative overflow-hidden">
            <h3 className="text-sm font-bold text-gray-900 dark:text-slate-200 mb-4">
              Quick Actions
            </h3>
            <div className="flex flex-col gap-3">
              <Link
                to="/matches"
                className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-slate-700/50 border border-gray-100 dark:border-slate-700 transition-colors group"
              >
          ...
          <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl shadow-lg p-6 text-white flex flex-col justify-between relative overflow-hidden min-h-[210px]">
            ...
            <div className="relative z-10">
              ...
              <p className="text-slate-400 text-sm leading-relaxed pr-6">
                Monitor and update scores for currently active games in
                real-time.
              </p>
            </div>
            <Link
              to="/admin/live-matches"
              className="bg-white text-slate-900 px-5 py-2.5 rounded-xl text-sm font-bold shadow hover:bg-slate-50 transition-colors w-max relative z-10 text-center"
            >
              Open Live Console
            </Link>
          </div>
